...
##            create_db() #this means that create_db is being called multiple times (it is in a for loop) no need for that


def populate_db(files, mTime):
##    itemList = cutMove(self)
    conn = sqlite3.connect('db_movedFiles.db')
    with conn:
        cur = conn.cursor()
        cur.execute("""INSERT INTO tbl_filenames (col_filename,col_modTime) VALUES (?,?)""", (files, mTime))
        conn.commit()
    conn.close()
##    db_contents
...
##def create_db(self): # shouldn't need self here
def create_db(): 
    conn = sqlite3.connect('db_movedFiles.db')
    with conn:
        cur = conn.cursor()
